...
export default class FourFloor {
  ...
  filterMesh() {
    for (const mesh of this.meshList) {
      if (
        !hasIncludeImportMeshName(mesh.name, 'fireFightingBoxF4') &&
        !hasIncludeImportMeshName(mesh.name, 'fireFightingCupboardF4') &&
        !hasIncludeImportMeshName(mesh.name, 'smogResponseF4')
      ) {
        if (mesh.type === 'Mesh') {
