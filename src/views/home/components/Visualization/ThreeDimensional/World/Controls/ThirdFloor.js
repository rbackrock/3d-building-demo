...
export default class ThirdFloor {
  ...
  filterMesh() {
    for (const mesh of this.meshList) {
      if (
        !hasIncludeImportMeshName(mesh.name, 'fireFightingBoxF3') &&
        !hasIncludeImportMeshName(mesh.name, 'fireFightingCupboardF3') &&
        !hasIncludeImportMeshName(mesh.name, 'smogResponseF3')
      ) {
        if (mesh.type === 'Mesh') {
